...
def build_inference_pipeline(
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # move to GPU
    ...
    inference_embedding = (
        Path(inference_embedding)
        if isinstance(inference_embedding, str)
        else inference_embedding
    )
    if inference_embedding.exists():
    ...
    process_inference_training_results(raw_results_file, cleaned_results_file)
    cleaned_inference_df = pd.read_csv(cleaned_results_file)

    # Step 8: Clean training data
    with tempfile.NamedTemporaryFile(delete=False, suffix=".csv") as temp_file:
        temp_file_name = temp_file.name
        process_inference_training_results(training_data_file, temp_file_name)
...
def run_inference_pipeline():
    ...
    Checks if the combined labeled dataset file already exists to skip redundant processing.
    """
    # * Check to initiate or skip pipeline
    if COMBINED_CLEANED_OUTPUT_DATA_FILE.exists():
        logging.info(
            "Combined labeled dataset already exists. Skipping inference pipeline."
        )
    ...
    build_inference_pipeline(
        ...
        all_data_file=TRAINING_INFERENCE_DATA_FILE,
        training_data_file=TRAINING_DATA_FILE,
        ...
        combined_cleaned_results_file=COMBINED_CLEANED_OUTPUT_DATA_FILE,
    )
