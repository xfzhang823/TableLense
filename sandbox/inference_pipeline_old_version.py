...
def build_inference_pipeline(
    ...
    model_nn = load_model(MODEL_PTH_FILE, input_dim, device).to(
        device
...
def main():
    ...
    all_data = PREPROCESSED_ALL_DATA_FILE  # all data
    training_data = (
        TRAINING_DATA_FILE  # training (including both train/test, already labeled)
    )
    filtered_data = (
        filtered_production_data_file  # production minus training data (not labeled)
    )
    embeddings = (
        INFERENCE_EMBEDDINGS_PKL_FILE  # embeddings for inference (production data)
    )
    raw_results = RAW_INFERENCE_OUTPUT_DATA_FILE  # inference results - not cleaned yet
    cleaned_results = CLEANED_INFERENCE_OUTPUT_DATA_FILE  # inference results - cleaned
    combined_and_cleaned_results = (
        COMBINED_INFERENCE_OUTPUT_DATA_FILE  # cleaned inference + cleaned training
    )
